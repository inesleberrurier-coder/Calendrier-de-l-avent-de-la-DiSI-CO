...
<script>
// -----------------------------
// Génération & verrouillage par date réelle
// -----------------------------

const calendar = document.getElementById('calendar');

// Utilitaire : obtient le jour actuel en local (1..31)
function getCurrentDayNumber() {
  const now = new Date();
  return now.getDate();
}

// Génère les cases 1..24 et bloque les futures
...
// -----------------------------
// Déverrouillage automatique à minuit (sans rechargement)
// -----------------------------

function scheduleMidnightRefresh() {
  const now = new Date();
  // Prochain minuit local : créer date avec lendemain 00:00:00
  const nextMidnight = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 0, 0, 1, 0);
  const ms = nextMidnight - now;
  // Planifier la régénération à minuit + 1s
  setTimeout(() => {
    generateCalendar();
    // Relancer la planification pour les prochains jours
    scheduleMidnightRefresh();
  }, ms);
  // Sécurité : aussi vérifier chaque minute (en cas de changement d'horloge)
  setInterval(() => {
    // si une case verrouillée devrait maintenant être déverrouillée, on regenère
    const currentDay = getCurrentDayNumber();
    const locked = document.querySelectorAll('.day.locked');
    if(locked.length > 0){
      // vérifier si au moins une locked a un numéro <= currentDay
...
// -----------------------------
// Popup & contenus (ton contenu d'origine)
// -----------------------------
function openPopup(day){
  ...
  switch(day){
    ...
    case 4: box.innerHTML+=`<h2>Jour 4</h2><p>Le site des Marsauderies accueille chaque année des nouveaux moutons 🐑</p><strong style='color:red;'>Quiz :</strong> Combien d’agneaux la DiSI CO a eu ce printemps ?</p>
      ...
      <p id='res4'></p>`; break;
    case 5: box.innerHTML+=`<h2>Jour 5</h2><p><strong style='color:red;'>Quiz :</strong> Le mois dernier a eu lieu le mois de l'innovation publique avec pour thème principal l'IA🤖 </p>Savez vous en quelle année l’IA a été créée ?</p>
